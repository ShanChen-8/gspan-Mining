    ...
    for DfsCode, PDfsCoLi in projected.items():
        ...
        for key, val in projected_new.items():
            ...
            if dfs_min == key and len(val) >= supp_thres:
                ...
                save_mined_graph_incrementally(inti_key, len(val), "mined_subgraphs4_2_incrementally.json")
...
def SubMiningTask(args):
    ...
    """
    key, val, G, supp_thres,i,length = args  # 解包参数
    ...
    SubMining(minedSubGraphs2, projected_filt_, G, supp_thres)  # 调用子图挖掘函数
...
def ParallelSubMining(minedSubGraphs, projected_filt, G, supp_thres):
    ...
    args_list = [(key, val, G, supp_thres, i,len(projected_filt)) for i, (key, val) in enumerate(projected_filt.items(), start=1)]
...
if __name__ == '__main__':
    ...
    # 调用保存函数
    output_file_path = "mined_subgraphs4_2_onetime.json"
