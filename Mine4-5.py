...
def SubMining(task_id, projected, G, supp_thres, out_filename):
    if not projected:
        return  # 原地修改
        ...
        for key, val in projected_new.items():
            ...
            if dfs_min == key and len(val) >= supp_thres:
                ...
                if len(inti_key) < 11:  # 最多到10阶子图
                    ...
                    print('第{}任务挖掘到{}阶子图{}，，支持度{}'.format(task_id,len(inti_key), inti_key, len(val)))
                    print(f'此子图处于projected中的：{t}/{len(projected_new)}')

        # 在最小频繁集上继续挖掘
        SubMining(task_id, projected_filt, G, supp_thres, out_filename)
        print('递归退出，正常结束任务前必有递归退出')
...
def SubMiningTask(args):
    ...
    try:
        G = shared["Graph"]  # 尝试获取图对象
        print("Successfully accessed Graph")
    ...
    try:
        SubMining(i, projected_filt_, G, supp_thres, out_filename)  # 使用全局 G
...
def ParallelSubMining(minedSubGraphs, projected_filt, G_local, supp_thres):
    ...
    # 准备参数列表，不传递 G_local
    # args_list = [
    #     (key, val, supp_thres, i, len(projected_filt), f"./INT1-50(<=10)/mined_subgraphs_{i}.json")
    #     for i, (key, val) in enumerate(projected_filt.items(), start=1)
    # ]
    args_list=[]
    for i, (key, val) in enumerate(projected_filt.items(), start=1):
        if 1 <=i <=50:
            args_list.append((key, val, supp_thres, i, len(projected_filt), f"./INT1-50(<=10)/mined_subgraphs_{i}.json"))
    # 使用 Manager 创建共享对象
    with Manager() as manager:
        ...
        with Pool(processes=25, initializer=init_pool, initargs=(shared_memory,)) as pool:
...
if __name__ == '__main__':
    ...
    projected = Projected()
    G = ReadGraph("/home/course/shanchen/Mining/merged_graph.data")  # 读取图
    supp_thres = 10000
    ...
    for i, (key, val) in enumerate(projected_filt.items()):
        inti_key = val[0].intigralKey()
        if 1 <= i+1 <=50:
            save_mined_graph_incrementally(inti_key, len(val), f"./INT1-50(<=10)/mined_subgraphs_{i + 1}.json")
    ...
    # # 保存最终结果
    # output_file_path = "./INT1-/mined_subgraphs4_4_onetime1.json"
