...
#     global G
#     G = graph
#     print("init_pool has been called and global G is set.")
class Edge():
...
def SubMiningTask(args):
    ...
    """
    key, val, G, supp_thres,i,length,out_filename = args  # 解包参数
    ...
    SubMining(minedSubGraphs2, projected_filt_, G, supp_thres,out_filename)  # 调用子图挖掘函数
    return minedSubGraphs2  # 返回结果

def ParallelSubMining(minedSubGraphs, projected_filt, G, supp_thres):
    """
    使用多进程加速对 projected_filt 的处理。
    """
    # args_list = [(key, val, G, supp_thres) for key, val in projected_filt.items()]  # 准备参数列表
    args_list = [(key, val, G, supp_thres, i,len(projected_filt),f"mined_subgraphs_{i}.json") for i, (key, val) in enumerate(projected_filt.items(), start=1)]
    with Pool(processes=cpu_count()) as pool:  # 创建进程池
        results = pool.map(SubMiningTask, args_list)  # 并行执行任务
...
if __name__ == '__main__':
    ...
    for i,(key ,val) in enumerate(projected.items()):
        if len(val) >=supp_thres:# 挖掘到的子图
            ...
            inti_key=val[0].intigralKey()
            minedSubGraphs[inti_key] = len(val)
    for i,(inti_key,val) in enumerate(projected_filt.items()):
        save_mined_graph_incrementally(inti_key, len(val), f"mined_subgraphs_{i+1}.json")

    ParallelSubMining(minedSubGraphs, projected_filt, G, supp_thres)
    endtime=time.time()
    print(f'time:{starttime}-{endtime}')
    # 调用保存函数0
    output_file_path = "mined_subgraphs4_3_onetime.json"
