...
def MultiFuncPdfsCode(args):
    """
    并行处理每一个 p_dfs_code，返回 (DfsCoCp, PCoCp) 的元组列表。
    打印当前线程的序号及总线程数。
    """
    index, p_dfs_code, G, total_threads = args  # 解包传递的参数
    process_info = current_process()
    print(f"正在处理任务： {index + 1}/{total_threads} process 名为{process_info.name} (PID: {process_info.pid})")
...
def SubMining(minedSubGraphs, projected, G, supp_thres):
    ...
    for DfsCode, PDfsCoLi in projected.items():
        ...
        with Pool(processes=cpu_count()) as pool:
            # 为每个任务附加索引和总线程数
            args_list = [(i, p_dfs_code, G, len(PDfsCoLi)) for i, p_dfs_code in enumerate(PDfsCoLi)]
            # 映射所有任务到并行函数
            results = pool.map(MultiFuncPdfsCode, args_list)
            pool.close() # 关闭任务提交入口，不允许再添加新的任务，但不影响已经提交的任务运行。
            pool.join()

        # 处理结果列表，合并到 projected_new 中
        for sublist in results:
            for DfsCoCp, PCoCp in sublist:
...
if __name__=="__main__":  # 在linux中运行需要这个，识别为程序入口
    ...
    SubMining(minedSubGraphs,projected_filt,G,supp_thres)
    # 调用保存函数
    output_file_path = "mined_subgraphs_2.json"
